...
(() => {
  ...
  var require_schedule = __commonJS({
    ...
    `;
        ...
        initErrorHandling() {
          window.electronAPI.onScheduleError((error) => {
            const errorMessage = this.formatErrorMessage(error);
            notification_1.notification.show(errorMessage, "error");
    ...
    `;
        ...
        startCountdownTimer(taskElement, nextRunTime) {
          ...
          const timerId = setInterval(updateCountdown, 6e4);
          taskElement.timerData = timerId;
          updateCountdown();
        }
        // 在任务删除时清理定时器
        cleanupTask(taskId) {
          ...
          if (!taskElement)
            return;
          const timerId = taskElement.timerData;
          if (timerId) {
        ...
        async exportTasks() {
          try {
            ...
            a.href = url;
            a.download = `schedule-tasks-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.json`;
          ...
          try {
            ...
            input.onchange = async (e) => {
              ...
              reader.onload = async (e2) => {
                try {
                  const tasks = JSON.parse(e2.target?.result);
                  ...
                  notification_1.notification.show("\u4EFB\u52A1\u5BFC\u5165\u6210\u529F", "success");
                  await this.loadTasks();
                } catch (error) {
                  notification_1.notification.show("\u5BFC\u5165\u5931\u8D25", "error");
                }
              };
    ...
    `;
        ...
        processTemplateMessage(message) {
          ...
          return message.replace("{date}", now.toLocaleDateString()).replace("{weather}", "\u5929\u6C14\u4FE1\u606F\u5F85\u66F4\u65B0");
        }
        showTemplates() {
          document.querySelector(".template-dropdown")?.classList.toggle("show");
        }
        initTaskStatusUpdate() {
        ...
        bindRepeatTypeEvents() {
          ...
          const weeklyOptions = document.querySelector(".repeat-weekly");
          const monthlyOptions = document.querySelector(".repeat-monthly");
          ...
          const monthlyDate = document.querySelector(".monthly-date");
          const monthlyWeek = document.querySelector(".monthly-week");
  ...
  var require_config_manager = __commonJS({
    "dist/renderer/modules/config-manager.js"(exports) {
      ...
      var ConfigManager = class {
        ...
        async updateSchedule(taskId, updates) {
          try {
            const schedules = await this.getSchedules();
